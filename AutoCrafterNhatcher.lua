...
do
    ...
    --                         Configuration & State
    --================================================================================--
    FarmModule.CONFIG_FILE_NAME = "CombinedFarmAndSeller_v14_SyncCheck.json"
    ...
    FarmModule.config = {
        maxWeightToSell = 4,
        targetEggCount = 3,
        cycleCounter = 0, -- For the sync check
        sellablePets = {
    ...
    function FarmModule:SaveConfig()
        ...
        local configToSave = {
            ...
            cycleCounter = self.config.cycleCounter
        }
    ...
    function FarmModule:LoadConfig()
        ...
        if success and fileData then
            ...
            if success2 and typeof(decodedData) == "table" then
                ...
                self.config.cycleCounter = decodedData.cycleCounter or 0
    ...
    function FarmModule:PerformOneCraftCycle()
        local success, err = pcall(function()
            ...
            self.config.cycleCounter = self.config.cycleCounter + 1
            ...
            if self.config.cycleCounter % 3 == 0 then
                ...
                if craftSuccess then
                    ...
                    self:SaveConfig(); task.wait(1); self.TeleportService:Teleport(game.PlaceId)
                else
                    warn("Craft failed (desync detected). Cancelling and restarting cycle.")
                    self.CraftingService:FireServer("Cancel", DinoTable, "DinoEventWorkbench")
                    self.needsEggCheck = true
                end
            else
                print("Craft cycle " .. self.config.cycleCounter .. ". Skipping sync check.")
                self.CraftingService:FireServer("Craft", DinoTable, "DinoEventWorkbench")
                self:SaveConfig(); task.wait(1); self.TeleportService:Teleport(game.PlaceId)
            end
        end)
        if not success then warn("Craft Error:", err, "-- Off."); self.isEnabled = false; self:UpdateButtonState(); self:SaveConfig() end
    end
    ...
    function FarmModule:CreateGUI()
        ...
        closeBtn.MouseButton1Click:Connect(function() self:Toggle(); gui:Destroy(); _G.MyCombinedFarmScriptIsRunning = false end)
    end
    function M:Toggle()
